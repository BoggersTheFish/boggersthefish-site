...
export const projects: Project[] = [
  {
    ...
    status: "Live",
    summary: "Typed verification reasoner with candidate containment, exported TensionLM-style set evaluation, and preserved failure receipts.",
    shortDescription: "Runnable typed verification traces with TensionLM-side export set receipts.",
    ...
    evidence: "v1.6.0 evaluates multiple exported TensionLM-side samples. Malformed, unsupported, contradictory, reverse, and deeper-chain current-limit cases are preserved as failure reasons instead of hidden.",
    limits: "Still narrow: the exported sample set is small, the verifier remains parser-controlled, and this is not live TensionLM integration into TS-Reasoner. The deeper-chain current-limit case is the next repair target.",
    repoUrl: links.repos.tsReasoner,
    hfUrls: [],
    proofIds: [],
    installCommand: "git clone https://github.com/BoggersTheFish/TS-Reasoner-v0 && cd TS-Reasoner-v0",
    inspectCommand: "python3 -m unittest discover && python3 scripts/evaluate_tensionlm_export_set.py",
    currentState: "v1.6.0 release is live with TensionLM export set receipt assets.",
    nextMilestone: "TS-Reasoner v1.7.0: deeper-chain support repair.",
