...
export const projects: Project[] = [
  {
    ...
    status: "Live",
    summary: "Typed verification reasoner with candidate containment, exported TensionLM-style ingestion, and real exported sample receipts.",
    shortDescription: "Runnable typed verification traces with TensionLM-side candidate containment receipts.",
    ...
    method: "Represent reasoning work as typed tension channels over graph state. TensionLM-side outputs enter only as candidate data: TS-Reasoner verifies channel traces, resolver events, abstentions, and receipts.",
    evidence: "v1.5.0 adds the first real exported TensionLM-side sample path. Provenance is preserved, typed channels verify support, and model output remains candidate data rather than proof.",
    limits: "Still narrow: the exported sample set is tiny, the verifier remains parser-controlled, and this is not live TensionLM integration into TS-Reasoner.",
    repoUrl: links.repos.tsReasoner,
    hfUrls: [],
    proofIds: [],
    installCommand: "git clone https://github.com/BoggersTheFish/TS-Reasoner-v0 && cd TS-Reasoner-v0",
    inspectCommand: "python3 -m unittest discover && python3 scripts/evaluate_real_exported_tensionlm_sample.py",
    currentState: "v1.5.0 release is live with real exported TensionLM-side sample receipt assets.",
    nextMilestone: "TS-Reasoner v1.6.0: TensionLM export set evaluation.",
    ...
    notes: [
      ...
      "v1.5.0 proves a real TensionLM-side exported sample can cross into TS-Reasoner as candidate data, not proof.",
    ],
  },
  {
    slug: "ts-core",
  ...
  },
  {
    ...
    nextMilestone: "TS-Reasoner v1.6.0 export set evaluation where multiple TensionLM-side samples remain candidate data under typed verification.",
